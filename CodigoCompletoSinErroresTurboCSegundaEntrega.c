...
struct involucrados* buscarInvolucradoPorRut(struct involucrados **involucrados, int tam, char *rut) {
    ...
    for (i = 0; i < tam && involucrados[i] != NULL; i++) {
         if (involucrados[i]->persona != NULL) {
...
struct involucrados* buscarImputadoEnCausa(struct causa *c, char *rutImputado) {
    ...
    if (c == NULL || rutImputado == NULL || c->involucrados == NULL) {
        printf("Error: causa o RUT inválido.\n");
        return NULL;
    }
    for (i = 0; i < c->tamInvolucrados && c->involucrados[i] != NULL ; i++) {
...
int agregarInvolucrado(struct causa *causaDestinada, struct involucrados *nuevoInvolucrado){
    ...
    for(i = 0; i < causaDestinada->tamInvolucrados && causaDestinada->involucrados[i] != NULL; i++){
...
int agregarDenunciaMenu(struct ministerio *ministerio, char *rutDenunciante){
...
void cambiarTipoInvolucrado(struct ministerio *ministerio) {
    ...
    for (i = 0; i < c->tamInvolucrados && c->involucrados != NULL; i++) {
        inv = c->involucrados[i];

        if (inv->persona != NULL && strcmp(inv->persona->rut, rut) == 0) {
            encontrado = 1;
...
struct persona *busquedaBinariaInvolucradosPorRut(struct involucrados **array, int tamInvolucrados, char *rutBuscado) {
    ...
    izq = 0,
    der = tamInvolucrados - 1;

    while (izq <= der) { // mientras la izq no haya sobrepasado derecha

      medio = (izq + der) / 2; // si el tam es par la division entera dedonde hacia abajo o sea a la izquierda

      cmp = strcmp(array[medio]->persona->rut, rutBuscado);
        if (cmp == 0) {
            return array[medio]->persona;
...
void OrdenarSeleccionDirectaInvolucradosPorRut(struct involucrados **involucrados, int tam) {
    ...
    // asigna el i y ademas busca el menor hasta el ultimo dato del array
    for (i = 0; i < tam - 1; i++) {
        indiceMinimo = i;
        for (j = i + 1; j < tam; j++) {
            if (strcmp(involucrados[j]->persona->rut, involucrados[indiceMinimo]->persona->rut) < 0) {
                indiceMinimo = j;
            }
        }

        // si el indice del minimo no es igual al i actual (osea, existe un minimo) intercambia posiciones
        if (indiceMinimo != i) {
            temporal = involucrados[i];
            involucrados[i] = involucrados[indiceMinimo];
            involucrados[indiceMinimo] = temporal;
        }
    }
}


struct persona *ingresarAlSistema(struct ministerio *ministerio) {
    char palabra[100];
    int rol;
    struct persona *nuevaPersona;
    nuevaPersona = (struct persona *)malloc(sizeof(struct persona));
...
void mostrarRucDePersonaBuscada(struct ministerio *ministerio, char *rutBuscado)
{
    ...
    do
    {
        ...
        for (i = 0; i < actual->datosCausa->tamInvolucrados && actual->datosCausa->involucrados[i] != NULL; i++)
        {
            if (actual->datosCausa->involucrados[i]->persona != NULL && actual->datosCausa->involucrados[i]->persona->rut != NULL && strcmp(actual->datosCausa->involucrados[i]->persona->rut, rutBuscado) == 0)
            {
                encontrado = 1;
            }
        }
        ...
        {
            if (actual->datosCausa->denuncia->denunciante != NULL && actual->datosCausa->denuncia->denunciante->rut != NULL && strcmp(actual->datosCausa->denuncia->denunciante->rut, rutBuscado) == 0)
            {
                encontrado = 1;
            }
            if (actual->datosCausa->denuncia->denunciado != NULL && actual->datosCausa->denuncia->denunciado->rut != NULL && strcmp(actual->datosCausa->denuncia->denunciado->rut, rutBuscado) == 0)
            {
                encontrado = 1;
            }
        }
        ...
        while (actualCarpeta != NULL)
        {
            if (actualCarpeta->datosCarpeta != NULL && actualCarpeta->datosCarpeta->datosPersona != NULL && actualCarpeta->datosCarpeta->datosPersona->rut != NULL && strcmp(actualCarpeta->datosCarpeta->datosPersona->rut, rutBuscado) == 0)
            {
                encontrado = 1;
            }
...
void menuAgregar(struct ministerio *ministerio){
    ...
    while(1){
        ...
        /*Opcion 2 agregar denuncia */
            ...
            scanf(" %[^\n]", rutDenunciante);
            resultado = agregarDenunciaMenu(ministerio, rutDenunciante);
...
void menuPrincipalUsuario(struct ministerio *ministerio, struct persona *usuarioEnSistema) {
    int opcion;
    opcion = -1;
    while(opcion != 0) {
        // Imprime el menu principal de opciones
        ...
        printf("4. Ingresar una denuncia en su nombre.\n");
        printf("0. Salir\n");
        printf("Seleccione una opcion: ");
        scanf("%d", &opcion);
        (void)getchar(); // Limpia el salto de linea despues de scanf
        ...
        switch(opcion) {
            case 1:
                // Mostrar Datos de la persona
                    mostrarUsuarioPorRut(ministerio->personas, usuarioEnSistema->rut);
            break;
            case 2:
                // Mostrar Denuncias persona
                    mostrarDenunciaPorRut(ministerio->personas, usuarioEnSistema->rut);
            break;
            case 3:
                // Mostrar casos donde esta involucrado
                    mostrarRucDePersonaBuscada(ministerio, usuarioEnSistema->rut);
            break;
            case 4:
                // Ingresar denuncia
                    agregarDenunciaMenu(ministerio, usuarioEnSistema->rut);
            break;
            case 0:
                printf("\nSaliendo del sistema.\n");
            break;
...
int main(){
    struct persona *PersonaIngresada;
    ...
    PersonaIngresada = ingresarAlSistema(ministerio);
    if (PersonaIngresada->rol == 1) {
        printf("Ingresando al sistema...");
        menuPrincipalUsuario(ministerio, PersonaIngresada);
    }
    else if (PersonaIngresada->rol == 2 || PersonaIngresada->rol == 3) {
        printf("Ingresando al sistema...");
